...
def main(args=None):
    ...
    if args.attack_type == 'byzantine':
        ...
        # Get malicious client indices for DBA attack
        malicious_client_indices = [i for i, is_benign in enumerate(client_type) if not is_benign]
        
        # Get attack type from config
        attack_pattern = particial_cfg.attack.backdoor.evils
        
        # Validate DBA configuration
        if attack_pattern == 'dba_backdoor':
            num_malicious = len(malicious_client_indices)
            if num_malicious == 0:
                warnings.warn("DBA attack configured but no malicious clients. Skipping backdoor attack.")
            else:
                num_triggers = len(particial_cfg.attack.backdoor.trigger_position)
                if num_triggers < num_malicious:
                    warnings.warn(
                        f"DBA attack: Number of trigger positions ({num_triggers}) is less than "
                        f"number of malicious clients ({num_malicious}). Some clients will have no triggers."
                    )
                print(f"DBA Attack Configuration:")
                print(f"  - Malicious clients: {num_malicious} (indices: {malicious_client_indices})")
                print(f"  - Total trigger positions: {num_triggers}")
                print(f"  - Triggers per client: ~{num_triggers // num_malicious}")
        
        # Apply backdoor attack to training data
        ...
        # Apply backdoor attack to test data (full trigger for ASR evaluation)
